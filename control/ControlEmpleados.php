...
class ControlEmpleados {
    ...
    function consultarCodigoNombre() {
        $fkArea = $this->objEmpleados->getFkArea();
        //$idEmpleado= $this->objEmpleados->getIdEmpleado();
        //$this->objEmpleados->setIdEmpleado($idEmpleado);
        $objControlConexion = new ControlConexion();
       // $objControlConexion->abrirBd("localhost", "id16682389_jdiez", "Juandiez123*", "id16682389_mesa_ayuda");
        $objControlConexion->abrirBd("localhost", "root", "", "mesa_ayuda");
        $resultado = $objControlConexion->ejecutarSelect("SELECT IDEMPLEADO, NOMBRE FROM empleado WHERE fkAREA='" . $fkArea . "';");
        $matriz = array();
        $i = 0;
        if ($resultado) {
            while ($mostrar = $resultado->fetch_array()) {
                ...
                $objEmpleados = new Empleados($idEmpleado, $nombre, '', '', '', '', '', '', '', '', '');
